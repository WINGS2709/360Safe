...
ULONG Fake_LoadImageNotifyRoutine(ULONG CallIndex, PVOID ArgArray, PULONG ret_func, PULONG ret_arg)
{
	...
	if (HashNumber == Global_Hash_2)
	{
		if (dword_1B120 && !dword_1B128 && !dword_1B12C)
		{
			...
			//返回值：成功返回1  失败返回0
			if (HookPort_SetFakeKiSystemServiceAddress() == TRUE)
			{
				dword_1B128 = 1;
			}
			dword_1B120 = 0;
		...
		if (Global_Version_Win10_Flag)
		{
			...
			if (ApiPortProcessId)
			{
				if (NT_SUCCESS(PsLookupProcessByProcessId(ApiPortProcessId, &Process)))
				{
					...
					{
						g_HookPort_Nt_Win32k_Data.ShadowSSDTTable_Data.ShadowSSDT_GuiServiceTableBase = NULL;
					}
					//解除附加
...
PVOID HookPort_19230()
{
	...
	//初始化哈希值
	Global_Hash_1 = 0x42503C81;		//bd0001.sys 	百度安全
	Global_Hash_2 = 0x4D71E020;		//tsfltmgr.sys 	qq管家
	Global_Hash_3 = 0xB8178767;		//kisknl.sys 	金山毒霸
	Global_Hash_4 = 0x7848DA1;		//knbdrv.sys 	猎豹安全浏览器
	...
	)
{
	...
	PLARGE_INTEGER		ioBuf = NULL;			  //SystemBuff可以当做输入、输出 
	...
	//Type3InputBuffer = IrpStack->Parameters.DeviceIoControl.Type3InputBuffer;
	...
	case HOOKPORT_DEBUGMEASSAGE1:		 //(写)
	{
		//开启输出调试信息的（无视）
	...
	case HOOKPORT_DEBUGMEASSAGE2:        //(写)
	{
		//开启输出调试信息的（无视）
		if (inBufLength >= sizeof(ULONG))
		{
	...
	case HOOKPORT_DEBUGMEASSAGE3:         //(写)
	{
		//开启输出调试信息的（无视）
		if (inBufLength >= sizeof(ULONG))
		{
	...
	case HOOKPORT_DEBUGMEASSAGE4:          //(写)
	{
