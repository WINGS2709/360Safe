...
// 添加特殊白名单进程信息（R3与R0通讯的进程）,不存在则添加
//返回值：是1，不是0
NTSTATUS  Safe_InsertSpecialWhiteList_PID()
{
	PROCESS_SESSION_INFORMATION SessionInfo = { 0 };
	PROCESS_BASIC_INFORMATION ProcessInfo = { 0 };
	UCHAR ImageFileNameBuff[0x256] = { 0 };
	PEPROCESS	Process = NULL;
	SIZE_T ReturnLength = NULL;
	ULONG SelfSessionId = NULL;
	ULONG SpecialWhiteListNumber = NULL;
	...
	{
		//获取进程终端ID
		Status = ZwQueryInformationProcess(NtCurrentProcess(), ProcessSessionInformation, &SessionInfo, sizeof(PROCESS_SESSION_INFORMATION), &ReturnLength);
		if (NT_SUCCESS(Status))
		{
			SelfSessionId = SessionInfo.SessionId;
		}
		//获取进程信息
		...
		if (NT_SUCCESS(Status))
		{
			...
			if (NT_SUCCESS(Status))
			{
				...
				if (!_stricmp(&ImageFileNameBuff, "services.exe"))
				{
					SelfSessionId = SPECIALSIGN;
				}
			}
		}
	}
	...
	{
		//1、新增插入  白名单个数+1，成功返回TRUE（个数<=0xE），失败FALSE（个数>0xE）
		//2、已存在    无视，默认返回TRUE（成功）
		while (PsGetCurrentProcessId() != g_SpecialWhite_List.SpecialWhiteListPID[Index])
		{
			//假设是新的白名单信息就插入
			if (Index >= SpecialWhiteListNumber)
			{
				//白名单进程个数<=0xE
				if (SpecialWhiteListNumber <= SPECIALWHITELISTSIZE)
				{
					g_SpecialWhite_List.SpecialWhiteListPID[SpecialWhiteListNumber] = PsGetCurrentProcessId();
					g_SpecialWhite_List.SpecialWhiteListSessionId[SpecialWhiteListNumber] = SelfSessionId;
					//白名单个数自增1
					g_SpecialWhite_List.SpecialWhiteListNumber++;
					//成功返回
