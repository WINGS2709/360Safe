...
//************************************     
// 函数名称: Safe_RemoveDesiredAccess_OpenService     
// 函数说明：访问敏感驱动服务路径进行降权处理  
// IDA地址 ：
// 作    者：Mr.M    
// 参考网址：
// 作成日期：2020/10/14     
// 返 回 值: ULONG     
// 参    数: IN PVOID In_SendMessage			[In]基地址
// 参    数: IN ULONG In_ServiceName_Offset     [In]服务名字偏移
// 参    数: IN ULONG In_Flag                   [In]0（宽字节 W）1（多字节 A）
//************************************  
ULONG NTAPI Safe_RemoveDesiredAccess_OpenService(IN PVOID In_SendMessage, IN ULONG In_ServiceName_Offset, IN ULONG In_Flag)
{
	ULONG       Status = NULL;
	ULONG	    Result = NULL;
	ACCESS_MASK Local_DesiredAccess_OpenService =	// 0x610D0122
		(GENERIC_WRITE | GENERIC_EXECUTE) |			// 0x60000000：GENERIC_WRITE | GENERIC_EXECUTE
		...
		(SERVICE_USER_DEFINED_CONTROL) |			// 0x00000100：SERVICE_USER_DEFINED_CONTROL
		(SERVICE_STOP) |							// 0x00000020：SERVICE_STOP 
		(SERVICE_CHANGE_CONFIG);					// 0x00000002：SERVICE_CHANGE_CONFIG 
	ULONG		Protect_SysMax = NULL;				//限制访问的路径个数
	PWCHAR		Protect_SysPath_w[] = {				//大数字是写死的
		L"MyDriver1.sys",
		L"QHActiveDefense",
		L"zhudongfangyu",						
	};
	PCHAR	    Protect_SysPath_a[] = {				//大数字是写死的
		 "MyDriver1.sys",
		 "QHActiveDefense",
		 "zhudongfangyu",
	};

	Protect_SysMax = In_Flag ? (sizeof(Protect_SysPath_a) / sizeof(ULONG)) : ((sizeof(Protect_SysPath_w) / sizeof(ULONG)));
	//1、调用者是非白名单保护进程
	if (!Safe_QueryWhitePID(PsGetCurrentProcessId()))
	{
		//2、禁止访问受保护服务
		//获取当前服务名称长度公式：In_SendMessage + In_ServiceName_Offset +0x4C
		SIZE_T Self_ServiceNameLen = *(ULONG*)((UCHAR*)In_SendMessage + In_ServiceName_Offset + 0x4C);
		//获取当前服务名称公式：    In_SendMessage + In_ServiceName_Offset +0x58
		PVOID  pSelf_ServiceName    = ((UCHAR*)In_SendMessage + In_ServiceName_Offset + 0x58);
		//2、1 判断下地址合法性
		for (ULONG Index = 0; Index < Protect_SysMax; Index++)
		{
			//2、1 获取自定义字符串长度
			SIZE_T Protect_SysPathLen = In_Flag ? strlen(Protect_SysPath_a[Index]) : wcslen(Protect_SysPath_w[Index]);
			//因为strlen or wcslen不算最后一个00 结尾的，所以这里要 + 1
			Protect_SysPathLen = Protect_SysPathLen + 1;
			//2、3 长度相同进行下一步判断
			if ((Self_ServiceNameLen == Protect_SysPathLen))
			{
				//2、4 然后判断名字
				Status = In_Flag ? (_strnicmp(pSelf_ServiceName, Protect_SysPath_a[Index], Self_ServiceNameLen)) : (_wcsnicmp(pSelf_ServiceName, Protect_SysPath_w[Index], Self_ServiceNameLen));
				if (!Status)
				{
					//2、5 符合条件降权
					//OpenServiceA的dwDesiredAccess：   InSendMessage（参数3） + ServiceName_Offset  +  0x5B（固定值）+ 驱动长度(A) & 0xFFFFFFFC
					//OpenServiceW的dwDesiredAccess：   InSendMessage（参数3） + ServiceName_Offset  +  0x5B（固定值）+ 驱动长度(W * 2) & 0xFFFFFFFC
					Self_ServiceNameLen = In_Flag ? (Self_ServiceNameLen) : (Self_ServiceNameLen * 2);
					//4字节对齐
					PVOID pSelf_ServiceDesiredAccess = ((ULONG)((UCHAR*)In_SendMessage + In_ServiceName_Offset + 0x58 + Self_ServiceNameLen)) & 0xFFFFFFFC;
					if (!myProbeWrite(pSelf_ServiceDesiredAccess, sizeof(PVOID), sizeof(CHAR)))
					{
						//阉割权限
						if (*(ULONG*)pSelf_ServiceDesiredAccess & Local_DesiredAccess_OpenService)
						{
							// 取反失去了非常重要的属性SERVICE_STOP ，后面无法卸载
							*(ULONG*)pSelf_ServiceDesiredAccess &= ~Local_DesiredAccess_OpenService;
						}
						//提供默认权限
						if (!*(ULONG*)pSelf_ServiceDesiredAccess)
						{
							*(ULONG*)pSelf_ServiceDesiredAccess = SERVICE_QUERY_STATUS;
						}
					}
					else
					{
						KdPrint(("ProbeWrite(Safe_RemoveDesiredAccess_OpenService：pSelf_ServiceDesiredAccess) error \r\n"));
						continue;
					}
					break;
				}
...
NTSTATUS NTAPI Safe_RPCDispatcher(IN PVOID In_SendMessage, IN HANDLE In_PortHandle)
{
	...
	if (Safe_CmpPortName(In_PortHandle, &NtsvcsString))
	{
		...
		switch (MessageType)
		{
			...
			case OPENSERVICEW_TYPE:
			{
				//禁止打开敏感驱动服务(降权)
				Safe_RemoveDesiredAccess_OpenService(In_SendMessage, ServiceName_Offset, FALSE);
				break;
			}
			case OPENSERVICEA_TYPE:
			{
				//禁止打开敏感驱动服务(降权)
				Safe_RemoveDesiredAccess_OpenService(In_SendMessage, ServiceName_Offset, TRUE);
				break;
			}
			default:
			{
	...
	//低版本
	if (!g_Win2K_XP_2003_Flag && MessageType == QUERYSERVICECONFIGA_TYPE)
	{
		//不感兴趣直接无视
	}
	return Result;
...
NTSTATUS NTAPI Safe_SetImagePathString(IN PUNICODE_STRING In_SysString, IN HANDLE KeyHandle, OUT PUNICODE_STRING Ou_ImagePathString)
{
	...
	//3、填充字符串 输出
	if (Ou_ImagePathString->Buffer && Ou_ImagePathString->Length)
	{
		switch (Switch_Flag)
		{
			case 0:		
...
// 返 回 值: BOOLEAN NTAPI     
// 参    数: IN HANDLE In_KeyHandle        [In]句柄
// 参    数: IN BOOLEAN In_a2_Flag         [In]标志位
// 参    数: IN ULONG In_a3                [In]未知
// 参    数: IN ULONG In_a4                [In]未知
// 参    数: IN ULONG In_a5                [In]未知
// 参    数: IN ULONG In_ErrorCode         [In]错误码，R3根据错误码显示不同的警告信息
//************************************  
BOOLEAN NTAPI Safe_ProtectRegKey(IN HANDLE In_KeyHandle, IN BOOLEAN In_a2_Flag, IN ULONG In_a3, IN ULONG In_a4, IN ULONG In_a5, IN ULONG In_ErrorCode)
{
	BOOLEAN					 Result = TRUE;
	HANDLE					 Object = NULL;
	...
	ULONG					 Tag = 0x206B6444u;
	NTSTATUS				 Status = STATUS_SUCCESS;
	ULONG					 Protect_Max = NULL;										//限制访问的路径个数
	POBJECT_NAME_INFORMATION pFileNameInfo = NULL;										//new出来的空间，需要释放
	UNICODE_STRING			 DestinationString = { 0 };									//RtlUpcaseUnicodeString转换后的，需要RtlFreeUnicodeString释放
	PWCHAR		             Protect_Path[] = {									
		L"SOFTWARE\\MICROSOFT\\WINDOWS\\CURRENTVERSION\\RUN",							//开机自启动
		L"SOFTWARE\\MICROSOFT\WINDOWS\\CURRENTVERSION\\EXPLORER\\STARTUPAPPROVED\\RUN",	//开机自启动
		L"POLICIES\\EXPLORER\\DISALLOWRUN",												//限制软件运行的目录
		L"IMAGE FILE EXECUTION OPTIONS",												//镜像劫持
	};
	Protect_Max = sizeof(Protect_Path) / sizeof(ULONG);
	//1、调用者是保护进程放行
	if (Safe_QueryWhitePID(PsGetCurrentProcessId()))
	{
		return Result;
	...
	//先将字符串转换成大写
	Status = RtlUpcaseUnicodeString(&DestinationString, &pFileNameInfo->Name, TRUE);
	if (!NT_SUCCESS(Status))
	{
		ExFreePool(pFileNameInfo);
		return Result;
	}
	for (ULONG Index = 0; Index < Protect_Max; Index++)
	{
		//各种敏感路径
		if (wcsstr(DestinationString.Buffer, Protect_Path[Index]))
		{
			//打包信息发给送R3
			//这里应该根据每种不同的选项发送不同的错误码，这里我偷懒了直接写成0
			Safe_18A72_SendR3(PsGetCurrentProcessId(), PsGetCurrentThreadId(), 0);
			//返回错误值，跳出循环结束
			Result = FALSE;
			break;
		}
