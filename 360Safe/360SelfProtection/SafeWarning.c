...
BOOLEAN NTAPI Safe_setevent_called_by_iodispatcher(IN HANDLE In_User_ThreadID, IN ULONG In_User_bypass_or_not)
{
	...
	for (wait_info = (WAITFOR_INFO *)g_wait_info_list.list.Flink;				//获取第一个链表
		wait_info != &g_wait_info_list;											//不等于表头
		wait_info = (WAITFOR_INFO *)wait_info->list.Flink						//指向下一组链表
		)
	{
		if (wait_info->tid == In_User_ThreadID)
		{
			wait_info->bypass_or_not = In_User_bypass_or_not;
			KeSetEvent(&wait_info->Event, 0, 0);
			break;
		}
	}
	//解锁
...
// 参    数: OUT PPE_HASH_DATA Out_SendHashData         [Out]计算后的哈希值
...
NTSTATUS NTAPI Safe_CheckSys_SignatureOrHash(IN HANDLE In_PorcessID, IN HANDLE In_ThreadID, IN PUNICODE_STRING In_pImagePathString, OUT PPE_HASH_DATA Out_SendHashData, OUT ULONG Out_PassFlag, IN ULONG In_Flag_a6)
{
	...
	Safe_158F8_Hash(&Out_SendHashData->Hash, KeyBuff);
	Out_SendHashData->PESize = ulLength;
	//6、查询列表中是否存在
	*(ULONG*)Out_PassFlag = Safe_QueryDrvmkDataList(Out_SendHashData);
_FunctionRet:
	result = Status;
	if (pBuff)
	{
...
NTSTATUS NTAPI Safe_1D044_SendR3(IN HANDLE In_PorcessID, IN HANDLE In_ThreadID, IN ULONG In_Flag, IN PUNICODE_STRING In_ImagePathString)
{
	...
	PE_HASH_DATA               Out_SendHashData = { 0 };
	...
	Status = Safe_CheckSys_SignatureOrHash(In_PorcessID, In_ThreadID, In_ImagePathString, &Out_SendHashData, &Pass_Flag, Flag_v4);
	//处理完毕释放
	if (pQuery_Pass)
	{
			...
			switch (Pass_Flag)
			{
				...
				case 2:		//检查的流程
				{
					...
					if (pQuery_Pass)
					{
						...
						RtlCopyMemory(pQuery_Pass->Hash,&Out_SendHashData,sizeof(PE_HASH_DATA) - sizeof(ULONG));//填写驱动哈希信息
						pQuery_Pass->Unknown_Flag_6 = 1;
						pQuery_Pass->Error_Flag = In_Flag;														//填写错误码
						...
						{
							Out_SendHashData.LoadDriver_Flag = 0;
							Safe_InsertDrvmkDataList(&Out_SendHashData);
							result = STATUS_SUCCESS;
						}
						else if (Flag_v16 == 1)			//添加黑名单进程列表，并返回错误值
						{
							Out_SendHashData.LoadDriver_Flag = 1;
							Safe_InsertDrvmkDataList(&Out_SendHashData);
							result = STATUS_ACCESS_DENIED;
						}
