...
//
ULONG g_SystemHotpatchInformationFlag;		//ZwSetSystemInformation函数使用，默认是1，会在Safe_IRP_Device_Control修改
...
{
	ULONG ListNumber;									// +0       保存数组个数
	WCHAR DosPath[0x200][DOSPATHSIZE / 2];				// +4       Dos路径数组，一共200个，每个最大长度520 / 2
	ULONG ArrayIndex[0x200];							// +0x41004 这个没卵用，保存DosPath在数组第几项，感觉多余的，从1开始算
	KSPIN_LOCK	SpinLock;								// 末尾 自旋锁 
...
typedef struct _QUERY_PASS_R0SENDR3_DATA
{
	...
	ULONG  Unknown_Flag_830;						//0x830						 解释：未知
