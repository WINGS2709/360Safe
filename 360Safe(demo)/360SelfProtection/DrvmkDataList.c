...
// 作    者：Mr.M      
// 返 回 值: ULONG NTAPI         
// 参    数: IN ULONG In_Hash          [In]哈希值
// 参    数: IN SIZE_T In_FileSize     [In]文件大小
//************************************ 
ULONG NTAPI Safe_QueryDrvmkDataList(IN ULONG In_Hash, IN SIZE_T In_FileSize)
{
	...
	if (DrvmkListNumber)
	{
		...
		{
			if (In_FileSize == g_Drvmk_List->Pe_Hash_Data[Index].PESize &&		//判断大小
				*(ULONG*)(In_Hash) == g_Drvmk_List->Pe_Hash_Data[Index].Hash[0]&&
				*(ULONG*)(In_Hash + 4) == g_Drvmk_List->Pe_Hash_Data[Index].Hash[1] &&
				*(ULONG*)(In_Hash + 8) == g_Drvmk_List->Pe_Hash_Data[Index].Hash[2] &&
				*(ULONG*)(In_Hash + 0xC) == g_Drvmk_List->Pe_Hash_Data[Index].Hash[3] 
				)
			{
				//找到跳出循环
				break;
...
// 参    数: IN ULONG In_Hash          [In]哈希值
// 参    数: IN SIZE_T In_FileSize     [In]文件大小
// 参    数: IN ULONG Pass_Flag        [In]放行or拦截标识：0放行 1拦截
//************************************ 
PVOID NTAPI Safe_InsertDrvmkDataList(IN ULONG In_Hash, IN SIZE_T In_FileSize, IN ULONG Pass_Flag)
{
	...
	NewIrql = KfAcquireSpinLock(&g_Drvmk_List->SpinLock);
	//判断名单个数,并且个数 <= 0x270E
	if (DrvmkListNumber && DrvmkListNumber <= DRVMKNUMBER)
	{
		//1、新增插入  名单个数+1 
		//2、已存在    无视
		while (
			In_FileSize != g_Drvmk_List->Pe_Hash_Data[Index].PESize &&				//判断大小
			Pass_Flag != g_Drvmk_List->Pe_Hash_Data[Index].LoadDriver_Flag &&	    //放行 or 拦截标识
			*(ULONG*)(In_Hash) != g_Drvmk_List->Pe_Hash_Data[Index].Hash[0] &&		//哈希值
			*(ULONG*)(In_Hash + 4) != g_Drvmk_List->Pe_Hash_Data[Index].Hash[1] &&
			*(ULONG*)(In_Hash + 8) != g_Drvmk_List->Pe_Hash_Data[Index].Hash[2] &&
			*(ULONG*)(In_Hash + 0xC) != g_Drvmk_List->Pe_Hash_Data[Index].Hash[3]
			)
		{
			...
			{
				//文件大小
				g_Drvmk_List->Pe_Hash_Data[Index].PESize = In_FileSize;
				//哈希值
				g_Drvmk_List->Pe_Hash_Data[Index].Hash[0] = *(ULONG*)(In_Hash);
				g_Drvmk_List->Pe_Hash_Data[Index].Hash[1] = *(ULONG*)(In_Hash + 4);
				g_Drvmk_List->Pe_Hash_Data[Index].Hash[2] = *(ULONG*)(In_Hash + 8);
				g_Drvmk_List->Pe_Hash_Data[Index].Hash[3] = *(ULONG*)(In_Hash + 0xC);
				//放行 or 拦截标识
				g_Drvmk_List->Pe_Hash_Data[Index].LoadDriver_Flag = Pass_Flag;
				//个数++
	...
	if (DrvmkListNumber)
	{
		for (Index = 0; Index < DrvmkListNumber; Index++)
		{
			if (In_FileSize == g_Drvmk_List->Pe_Hash_Data[Index].PESize &&		         //判断大小
				Pass_Flag == g_Drvmk_List->Pe_Hash_Data[Index].LoadDriver_Flag &&	     //放行 or 拦截标识
				*(ULONG*)(In_Hash) == g_Drvmk_List->Pe_Hash_Data[Index].Hash[0] &&       //哈希值
				*(ULONG*)(In_Hash + 4) == g_Drvmk_List->Pe_Hash_Data[Index].Hash[1] &&   //哈希值
				*(ULONG*)(In_Hash + 8) == g_Drvmk_List->Pe_Hash_Data[Index].Hash[2] &&   //哈希值
				*(ULONG*)(In_Hash + 0xC) == g_Drvmk_List->Pe_Hash_Data[Index].Hash[3]    //哈希值
				)
			{
				//清空退出进程的信息(后一个往前挪)
				for (ULONG i = Index; i <= DrvmkListNumber; i++)
				{
					g_Drvmk_List->Pe_Hash_Data[Index].PESize = g_Drvmk_List->Pe_Hash_Data[Index + 1].PESize;			       //文件大小
					g_Drvmk_List->Pe_Hash_Data[Index].LoadDriver_Flag = g_Drvmk_List->Pe_Hash_Data[Index + 1].LoadDriver_Flag; //放行 or 拦截标识
					g_Drvmk_List->Pe_Hash_Data[Index].Hash[0] = g_Drvmk_List->Pe_Hash_Data[Index + 1].Hash[0];				   //哈希值
					g_Drvmk_List->Pe_Hash_Data[Index].Hash[1] = g_Drvmk_List->Pe_Hash_Data[Index + 1].Hash[1];                 //哈希值
					g_Drvmk_List->Pe_Hash_Data[Index].Hash[2] = g_Drvmk_List->Pe_Hash_Data[Index + 1].Hash[2];                 //哈希值
					g_Drvmk_List->Pe_Hash_Data[Index].Hash[3] = g_Drvmk_List->Pe_Hash_Data[Index + 1].Hash[3];                 //哈希值
				}
				//保护进程个数-1
				--g_Drvmk_List->ListNumber;
				break;
